...
no.addEventListener('click', () => {
    ...
    localStorage.setItem("uiState", "1");
...
setTimeout( () => {
    ...
    logoBall.style.display = 'block'
    login.classList.add('loginAnimationFast')
    logo.classList.add('loginAnimationFast')
    logoBall.classList.add('loginAnimationFast')
    mainBar.classList.add('loginAnimationFast')
    paragraph.classList.add('moveFrontParagraph');
    button.style.animation = 'opacity 0.8s ease-out 800ms 1 normal forwards';
    button.innerText = 'Start'
}, {once: true});
}

// This below was from AI 
window.addEventListener("pageshow", () => {
    ...
    const fromDir = from.pathname.replace(/[^/]+$/, "");
    const hereDir = here.pathname.replace(/[^/]+$/, "");

    if (fromDir !== hereDir) return;

    const state = localStorage.getItem("uiState");
    if (state === "1" || state === "2") restoreMainState();
});
...
nav.forEach(navEl => {
    ...
    navEl.style.width = '115px'
    navEl.style.height = '95px'
    navEl.style.color = 'hsl(224, 75%, 38%)'
    navEl.style.fontSize = '20px'
    navEl.style.borderBottom = 'none'
    navEl.style.marginBottom = '5px'
}
...
const navOff = () => {
    ...
    navEl.style.color = 'hsl(225, 81%, 23%)'
    navEl.style.fontSize = '22px'
    navEl.style.borderBottom = 'none'
    navEl.style.marginBottom = '5px'
}
...
document.addEventListener('mousemove', (e) => {
    ...
    if (distance < 51) {
        ...
        navEl.addEventListener('mousedown', navClick);
        navEl.addEventListener('mouseup', navOff);
    } else if (distance > 51) {
        isActive = false
        navEl.classList.add('navCloseBy')
    navEl.style.backgroundColor = 'transparent'
    ...
    navEl.style.marginBottom = '5px'
        navEl.classList.remove('navCloseBy');
        navEl.removeEventListener('mousedown', navClick);
        navEl.removeEventListener('mouseup', navOff);
    }
})
